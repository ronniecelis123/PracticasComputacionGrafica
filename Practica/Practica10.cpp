...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// >> CAMBIO: Usar la altura variable 'currentDogHeight'
		...
		model = glm::rotate(model, glm::radians(-rotBall), glm::vec3(0.0f, 1.0f, 0.0f));
		// >> CAMBIO: Usar la altura variable 'currentBallHeight'
...
void Animation() {
	if (AnimBall) {
		rotBall -= 0.02f; // Una velocidad más rápida para que la animación se vea bien
	}
	else {
	...
	if (isHitting) {
		...
		if (hitTimer > 0.0f) {
			...
			float ballDipMaxHeight = 0.8f;  // Cuánto baja la pelota
